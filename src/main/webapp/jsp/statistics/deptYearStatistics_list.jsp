...
<div class="action  divaction">
	...
	<div class="pages">
	     <form action="/deptYearStatistics/findDeptYearStatisticsList" name="queryForm">
		...
		<table width="90%" border="0" cellspacing="0" cellpadding="0" class="list items">
	      ...
	      <c:forEach items="${pageSupport.items}" var="claimVouyearStatistics" begin="0" varStatus="s">
	      <tr>
	        <td>${claimVouyearStatistics.id}</td>
	        <td>￥${claimVouyearStatistics.totalCount}</td>
	        <td>${claimVouyearStatistics.year}年</td>
	       <%--  <td><s:property value="#claimVoucherStatistics.month"/>月</td>--%>
	        <td>${claimVouyearStatistics.department.name}</td>
	        <td>
	        ...
	        <a href="/deptYearStatistics/findDeptYearStatisticsDetail?currYear=${claimVouyearStatistics.year}&departmentId=${claimVouyearStatistics.department.id}">
