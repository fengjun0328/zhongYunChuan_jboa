...
<script type="text/javascript" src="${pageContext.request.contextPath}/statics/js/common.js">
</script>

<div class="action  divaction">
	<div class="t">月度统计列表</div>
	<div class="pages">
	     <form action="/deptMonthStatistics/getDeptStatisticsByMonth" name="queryForm">
	     		...
	     		<select name="year">
	       		...
	       		<select name="startMonth">
					<c:forEach var="month" step="1" begin="1" end="12">
						<option value="${month}" <c:if test="${startMonth==month}">selected="selected"</c:if>>${month}月份</option>
					</c:forEach>
				</select>
	       		<label for="end-time">结束月份</label>
	       		<select name="endMonth">
					<c:forEach var="month" step="1" begin="1" end="12">
						<option value="${month}"  <c:if test="${endMonth==month}">selected="selected"</c:if> >${month}月份</option>
					</c:forEach>
				</select>
		...
		<table width="90%" border="0" cellspacing="0" cellpadding="0" class="list items">
	      ...
	      <c:forEach items="${pageSupport.items}" var="claimVoucherStatistics" begin="0" varStatus="s">
	      <tr>
	        ...
	        <td>${claimVoucherStatistics.department.name}</td>
	        <td>
	        ...
	        <a href="/deptMonthStatistics/getDeptVoucherDetailByMonth?year=${claimVoucherStatistics.year}&selectMonth=${claimVoucherStatistics.month}&departmentId=${claimVoucherStatistics.department.id}">
